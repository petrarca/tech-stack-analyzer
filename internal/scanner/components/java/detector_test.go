...
func TestDetector_Detect_PomXMLWithLicense(t *testing.T) {
	detector := &Detector{}
...
<project>
	...
	payload := results[0]
	require.Len(t, payload.Licenses, 1, "Should detect one license from pom.xml")
	assert.Equal(t, "Apache-2.0", payload.Licenses[0].LicenseName)
	assert.Equal(t, "direct", payload.Licenses[0].DetectionType)
	assert.Equal(t, "pom.xml", payload.Licenses[0].SourceFile)
}

func TestDetector_Detect_PomXMLWithMultipleLicenses(t *testing.T) {
	detector := &Detector{}
...
<project>
	...
	payload := results[0]
	assert.Len(t, payload.Licenses, 2, "Should detect two licenses from pom.xml")

...
func TestDetector_Detect_DenoJsonLicense(t *testing.T) {
	detector := &Detector{}
  ...
  "license": "MIT"
	...
	payload := results[0]
	require.NotEmpty(t, payload.Licenses, "Should have license from deno.json")
	assert.Equal(t, "MIT", payload.Licenses[0].LicenseName)
	assert.Equal(t, "deno.json", payload.Licenses[0].SourceFile)
}

func TestDetector_Detect_DenoJsonOnly(t *testing.T) {
	detector := &Detector{}
  ...
  "license": "Apache-2.0"
	...
	require.Len(t, results, 1, "Should detect Deno project from deno.json alone")

	payload := results[0]
	assert.Equal(t, "my-deno-app", payload.Name)
	assert.Contains(t, payload.Tech, "deno", "Should have deno as primary tech")
	require.NotEmpty(t, payload.Licenses, "Should have license from deno.json")
	assert.Equal(t, "Apache-2.0", payload.Licenses[0].LicenseName)
}

func TestDetector_Detect_DenoJsonCompoundLicense(t *testing.T) {
	detector := &Detector{}

	denoJsonContent := `{
  "name": "dual-licensed-app",
  "license": "MIT OR Apache-2.0"
	...
	payload := results[0]
	assert.Len(t, payload.Licenses, 2, "Should have 2 licenses from compound expression")
...
func TestDetector_Detect_DenoJsonNoNameNoLicense(t *testing.T) {
	detector := &Detector{}
  ...
  "compilerOptions": {
	...
	assert.Empty(t, results, "Should not create payload for deno.json with no name and no license")
}

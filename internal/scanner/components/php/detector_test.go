...
func TestDetector_Detect_LicenseExpression(t *testing.T) {
	detector := &Detector{}

	// composer.json with SPDX expression
	composerContent := `{
    "name": "example/dual-licensed",
	...
	// Should parse expression into two separate licenses
	assert.Len(t, payload.Licenses, 2, "Should have two licenses from expression")
	...
	for _, lic := range payload.Licenses {
		...
		assert.Equal(t, "composer.json", lic.SourceFile)
		assert.Equal(t, "MIT OR Apache-2.0", lic.OriginalLicense)
	}

	assert.True(t, licenseNames["MIT"], "Should have MIT license")
	assert.True(t, licenseNames["Apache-2.0"], "Should have Apache-2.0 license")
}

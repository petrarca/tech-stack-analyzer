...
func TestExtractGemspecLicenses(t *testing.T) {
	tests := []struct {
		name     string
		content  string
		expected []string
		...
		{
			name: "single license with spec.license",
  ...
  spec.license = "MIT"
		...
		{
			name: "single license with s.license",
  ...
  s.license = 'Apache-2.0'
		...
		{
			name: "multiple licenses with spec.licenses",
			content: `Gem::Specification.new do |spec|
  spec.name = "mylib"
  spec.licenses = ["MIT", "GPL-2.0"]
		...
		{
			name: "licenses array takes precedence",
			content: `Gem::Specification.new do |spec|
  spec.name = "mylib"
  spec.licenses = ["BSD-3-Clause", "Apache-2.0"]
  spec.license = "MIT"
end`,
			expected: []string{"BSD-3-Clause", "Apache-2.0"},
		},
		{
  ...
  spec.version = "1.0.0"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := extractGemspecLicenses(tt.content)
			if tt.expected == nil {
...
func TestDetector_Detect_GemfileWithGemspecLicense(t *testing.T) {
	detector := &Detector{}
...
`
  ...
  spec.license = "MIT"
	...
	provider := &MockProvider{
		files: map[string]string{
			"/project/Gemfile":       gemfileContent,
			"/project/mylib.gemspec": gemspecContent,
		},
	}
	...
	files := []types.File{
		{Name: "Gemfile", Path: "/project/Gemfile"},
		{Name: "mylib.gemspec", Path: "/project/mylib.gemspec"},
	}
	...
	payload := results[0]
	require.NotEmpty(t, payload.Licenses, "Should have license from gemspec")
	assert.Equal(t, "MIT", payload.Licenses[0].LicenseName)
	assert.Equal(t, "mylib.gemspec", payload.Licenses[0].SourceFile)
}

func TestDetector_Detect_GemfileWithGemspecMultipleLicenses(t *testing.T) {
	detector := &Detector{}
...
`
  ...
  spec.licenses = ["MIT", "Apache-2.0"]
	...
	provider := &MockProvider{
		files: map[string]string{
			"/project/Gemfile":         gemfileContent,
			"/project/duallib.gemspec": gemspecContent,
		},
	}
	...
	files := []types.File{
		{Name: "Gemfile", Path: "/project/Gemfile"},
		{Name: "duallib.gemspec", Path: "/project/duallib.gemspec"},
	}
	...
	payload := results[0]
	assert.Len(t, payload.Licenses, 2, "Should have 2 licenses from gemspec")

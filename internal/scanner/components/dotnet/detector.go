...
func (d *Detector) createDotNetPayload(project *parsers.DotNetProject, file types.File, currentPath, basePath string) *types.Payload {
	...
	// Process license: prefer PackageLicenseExpression, fall back to PackageLicenseUrl
	if project.License != "" {
		d.processLicense(project.License, file.Name, payload)
	} else if project.LicenseUrl != "" {
		d.processLicenseUrl(project.LicenseUrl, file.Name, payload)
	}

	return payload
}

// processLicense handles license processing for .csproj PackageLicenseExpression
func (d *Detector) processLicense(rawLicense string, sourceFile string, payload *types.Payload) {
	licensenormalizer.ProcessLicenseExpression(rawLicense, sourceFile, payload)
}

// licenseUrlMappings maps common NuGet PackageLicenseUrl values to SPDX identifiers.
// This is a best-effort mapping for the deprecated PackageLicenseUrl field.
var licenseUrlMappings = map[string]string{
	...
	"https://licenses.nuget.org/MPL-2.0":          "MPL-2.0",
	"https://licenses.nuget.org/LGPL-2.1-only":    "LGPL-2.1-only",
	"https://licenses.nuget.org/LGPL-3.0-only":    "LGPL-3.0-only",
	"https://licenses.nuget.org/GPL-2.0-only":     "GPL-2.0-only",
	"https://licenses.nuget.org/GPL-3.0-only":     "GPL-3.0-only",
}

// processLicenseUrl handles the deprecated PackageLicenseUrl field as a fallback.
// Maps known license URLs to SPDX identifiers.
func (d *Detector) processLicenseUrl(licenseUrl string, sourceFile string, payload *types.Payload) {
	licenseUrl = strings.TrimSpace(licenseUrl)
	if licenseUrl == "" {
		return
	}

	if spdx, ok := licenseUrlMappings[licenseUrl]; ok {
		licensenormalizer.ProcessLicenseExpression(spdx, sourceFile, payload)
		return
	}

	// Also try https://licenses.nuget.org/<SPDX-ID> pattern dynamically
	const nugetPrefix = "https://licenses.nuget.org/"
	if strings.HasPrefix(licenseUrl, nugetPrefix) {
		spdxID := strings.TrimPrefix(licenseUrl, nugetPrefix)
		if spdxID != "" {
			licensenormalizer.ProcessLicenseExpression(spdxID, sourceFile, payload)
			return
		}
	}
}

...
func TestDetector_Detect_CsprojWithLicense(t *testing.T) {
	detector := &Detector{}
  ...
  <ItemGroup>
	...
	provider := &MockProvider{
		files: map[string]string{
			"/project/MyLib.csproj": csprojContent,
		},
	}
	...
	files := []types.File{
		{Name: "MyLib.csproj", Path: "/project/MyLib.csproj"},
	}
	...
	payload := results[0]
	require.Len(t, payload.Licenses, 1, "Should detect license from .csproj")
	assert.Equal(t, "MIT", payload.Licenses[0].LicenseName)
	assert.Equal(t, "direct", payload.Licenses[0].DetectionType)
	assert.Equal(t, "MyLib.csproj", payload.Licenses[0].SourceFile)
}

func TestDetector_Detect_CsprojWithLicenseExpression(t *testing.T) {
	detector := &Detector{}

	csprojContent := `<Project Sdk="Microsoft.NET.Sdk">
  <PropertyGroup>
    <TargetFramework>net8.0</TargetFramework>
    <AssemblyName>DualLib</AssemblyName>
	...
	provider := &MockProvider{
		files: map[string]string{
			"/project/DualLib.csproj": csprojContent,
		},
	}
	...
	files := []types.File{
		{Name: "DualLib.csproj", Path: "/project/DualLib.csproj"},
	}
	...
	payload := results[0]
	assert.Len(t, payload.Licenses, 2, "Should parse dual license expression")

	licenseNames := make(map[string]bool)
	for _, lic := range payload.Licenses {
		licenseNames[lic.LicenseName] = true
...
func TestDetector_Detect_PackageLicenseUrl(t *testing.T) {
	detector := &Detector{}
  ...
  <PropertyGroup>
	...
	provider := &MockProvider{
		files: map[string]string{
			"/project/LegacyApp.csproj": csprojContent,
		},
	}
	...
	files := []types.File{
		{Name: "LegacyApp.csproj", Path: "/project/LegacyApp.csproj"},
	}
	...
	require.Len(t, results, 1)
	payload := results[0]
	require.NotEmpty(t, payload.Licenses, "Should detect license from PackageLicenseUrl")
	assert.Equal(t, "MIT", payload.Licenses[0].LicenseName)
}

func TestDetector_Detect_PackageLicenseExpressionTakesPrecedence(t *testing.T) {
	detector := &Detector{}

	// Both PackageLicenseExpression and PackageLicenseUrl present
	csprojContent := `<Project Sdk="Microsoft.NET.Sdk">
  <PropertyGroup>
    <AssemblyName>ModernApp</AssemblyName>
    <TargetFramework>net6.0</TargetFramework>
	...
	provider := &MockProvider{
		files: map[string]string{
			"/project/ModernApp.csproj": csprojContent,
		},
	}
	...
	files := []types.File{
		{Name: "ModernApp.csproj", Path: "/project/ModernApp.csproj"},
	}
	...
	require.Len(t, results, 1)
	payload := results[0]
	require.Len(t, payload.Licenses, 1, "Should have exactly one license (expression takes precedence)")
	assert.Equal(t, "Apache-2.0", payload.Licenses[0].LicenseName,
		"PackageLicenseExpression should take precedence over PackageLicenseUrl")
}

func TestDetector_Detect_PackageLicenseUrlKnownMapping(t *testing.T) {
	detector := &Detector{}

	csprojContent := `<Project Sdk="Microsoft.NET.Sdk">
  <PropertyGroup>
    <AssemblyName>LegacyApache</AssemblyName>
    <TargetFramework>net6.0</TargetFramework>
	...
	provider := &MockProvider{
		files: map[string]string{
			"/project/LegacyApache.csproj": csprojContent,
		},
	}
	...
	files := []types.File{
		{Name: "LegacyApache.csproj", Path: "/project/LegacyApache.csproj"},
	}

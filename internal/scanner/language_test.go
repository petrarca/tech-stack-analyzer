...
func TestDetectLanguage_NoReclassify(t *testing.T) {
	...
	// go-enry classifies .e as Eiffel by default
	lang = d.DetectLanguage("catalog.e", []byte("field1;field2;field3"))
	if lang != "Eiffel" {
...
func TestDetectLanguageWithType_LanguageAndType(t *testing.T) {
	...
	result := d.DetectLanguageWithType(filepath.Join(dir, "data", "catalog.e"), []byte("field1;field2"))
...
func TestDetectLanguageWithType_TypeOnly(t *testing.T) {
	...
	result := d.DetectLanguageWithType(filepath.Join(dir, "catalog.e"), []byte("field1;field2"))
	if result.Language != "" {
...
func TestDetectLanguageWithType_FolderScoped(t *testing.T) {
	...
	os.MkdirAll(filepath.Join(dir, "data", "catalogs"), 0o755)
	...
	result := d.DetectLanguageWithType(filepath.Join(dir, "data", "catalogs", "products.e"), []byte("field1;field2"))
	if result.Language != "CSV" {
		t.Errorf("data/catalogs/products.e: expected CSV, got %q", result.Language)
	}
	if result.TypeOverride != "data" {
		t.Errorf("data/catalogs/products.e: expected type data, got %q", result.TypeOverride)
	}
...
func TestDetectLanguageWithType_FirstMatchWins(t *testing.T) {
	...
	// Matches second rule
	result = d.DetectLanguageWithType(filepath.Join(dir, "other", "catalog.e"), nil)
	if result.Language != "CSV" {

...
// empty string when it is unresolved (a range, tag, or placeholder).
func resolvedVersion(v string) string {
	v = strings.TrimSpace(v)
	if v == "" || v == "latest" || v == "git" {
		return ""
	}
	// Reject npm/composer/python range and operator prefixes.
	if strings.ContainsAny(v, "^~><=*|") {
		return ""
	}
	if strings.HasPrefix(v, "git:") || strings.HasPrefix(v, "path:") {
		return ""
	}
	return v
}

...
func init() {
	...
	scanCmd.Flags().StringVar(&settings.MavenGraphSource, "maven-graph-source", settings.MavenGraphSource, "Source for Maven transitive dependency edges (with --dependency-graph direct|full): 'repo' crawls the configured Maven repo chain only (covers private artifacts; never contacts deps.dev), 'deps-dev' uses deps.dev for the public set and falls back to the repo crawl for private artifacts when a repo is configured, 'none' offline-only. Default follows --deps-dev. A committed dependency-tree.json always wins first.")

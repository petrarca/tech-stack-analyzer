...
### Dependency-graph test layers

The dependency-graph feature is tested at three levels, all in the default
(offline) `go test ./...` suite:

1. **Parser unit tests** -- each `Parse*Graph` producer tested against inline
   fixtures for off/direct/full modes and edge cases (multi-version, conflict
   resolution, scope, unresolved references).
2. **Real-lockfile fixture tests** (`internal/scanner/parsers/lockfile_fixtures_test.go`)
   -- the producers run against **real lockfiles** generated by the actual
   package managers (cargo, npm, pnpm, uv, bundler, composer, mix, poetry) and
   stored in `internal/scanner/parsers/testdata/lockfiles/`. These verify the
   parsers handle authentic tool output, not just hand-written strings.
3. **End-to-end scanner tests** (`internal/scanner/dependency_graph_test.go`)
   -- scan a temp project directory with the graph enabled and assert the edges
   in the payload tree, exercising the full wiring
   (detector -> `AttachLockfileGraph` -> resolver chain -> producer -> payload),
...
> flask, plug, ...). They are generated fresh from public packages -- never
> copied from internal or proprietary repositories. No internal package names,
> registry URLs, or paths may appear in a committed fixture.

...
- `--maven-central` - Enable the public Maven Central fallback for resolving Maven BOM/parent POM versions (default off). Ignored when `--maven-repo-url` is set.
- `--maven-repo-url`, `--maven-graph-source`, `--maven-local-repo`, `--maven-settings` - Maven-specific resolution against an internal/JFrog repository, including transitive resolution. See the [Maven guide](maven.md).

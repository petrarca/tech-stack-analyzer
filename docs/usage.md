...
- `--resolve-online` - Allow online dependency resolution (deps.dev) as a fallback for ecosystems without a committed resolved tree. Off by default; when enabled the scanner fans out over the component's declared dependencies, queries deps.dev for each one, and unions the results. Private or unknown deps are silently skipped (404). Edges are tagged `source: deps.dev`. A present local lockfile/tree always wins (local-first). Per deps.dev API docs, graph data is currently available for **npm, Cargo, Maven, and PyPI** only; other ecosystems fall through gracefully.

...
- `--dependency-graph` - Emit package-to-package dependency edges read from lockfiles: `off` (default), `direct` (root-to-direct edges only), or `full` (the full transitive graph). The full graph can be very large in big projects, so it is off by default. Produced for pnpm, npm, yarn, Cargo, poetry, uv, Ruby (`Gemfile.lock`), PHP (`composer.lock`), NuGet (`packages.lock.json`), and Go (`go.mod`; full graph from a pre-generated `go.mod.graph`). For Maven and Gradle (and any ecosystem) the scanner ingests a pre-generated resolved tree it never produces: `dependency-tree.json` (`mvn dependency:tree -DoutputType=json`), `gradle-dependencies.txt` (`gradle dependencies`), or a CycloneDX `bom.json` dependency-graph section. Each edge carries `source` (provenance) and, on direct edges, `scope` (prod/dev/optional/peer). Edges appear per component in the full tree and as a single deduplicated, sorted top-level `dependency_edges` array in the aggregate output.
- `--resolve-online` - Allow online dependency resolution (deps.dev) as a fallback for any ecosystem without a committed resolved tree. Off by default; when enabled the scanner fans out over the component's declared dependencies (already parsed from the manifest/lockfile), queries deps.dev for each one, and unions the results. Private or unknown deps are silently skipped; only published packages are resolved. Edges are tagged `source: deps.dev`. A present local lockfile/tree always wins (local-first). Works for any ecosystem whose package type is known to deps.dev (Maven, npm, PyPI, Cargo, Go, NuGet, RubyGems, CPAN, CRAN, Pub, Hex, Swift, Conan).
- `--resolve-online-endpoint` - Base URL for online resolution (default: public deps.dev `https://api.deps.dev`). Override with a deps.dev-API-compatible facade or mirror.

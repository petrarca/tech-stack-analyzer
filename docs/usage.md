...
- `--dependency-graph` - Emit package-to-package dependency edges read from lockfiles: `off` (default), `direct` (root-to-direct edges only), or `full` (the full transitive graph). The full graph can be very large in big projects, so it is off by default. Produced for pnpm, npm, yarn, Cargo, poetry, and uv lockfiles, and for Maven from a pre-generated `dependency-tree.json` (`mvn dependency:tree -DoutputType=json`; the scanner never runs Maven). Edges appear per component in the full tree and as a single deduplicated, sorted top-level `dependency_edges` array in the aggregate output.

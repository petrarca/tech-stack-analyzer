...
- `--resolve-online` - Allow online dependency resolution (deps.dev) as a fallback for manifest-only ecosystems (Maven, Gradle) that have no committed resolved tree. Off by default; when enabled it sends public package coordinates over the network and tags resulting edges `source: deps.dev`. A present local lockfile/tree always wins (local-first). Wired for Maven and Gradle.
